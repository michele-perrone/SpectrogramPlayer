    ...
    def forward(self, x, mag, max_length=None, repeat=1):
        ...
        for ii in range(repeat):
            for dnn in self.dnns:
                ...
                # B, F, T, 2
                waves = torch.istft(mag_replaced.permute(0, 2, 3, 1), n_fft=self.n_fft, hop_length=self.hop_length, window=self.window.data,
                        length=max_length, return_complex=False)
